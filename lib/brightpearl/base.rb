...
require 'brightpearl/connection'
require 'brightpearl/formats'
require 'brightpearl/schema'
require 'brightpearl/log_subscriber'
...
module Brightpearl
  ...
  class Base
    ...
    class << self
      ...
      # Gets the \api_version variable
      def api_version
        # Not using superclass_delegating_reader. See +site+ for explanation
        if defined?(@api_version)
          @api_version
        elsif superclass != Object && superclass.api_version
          superclass.api_version.dup.freeze
        end
      end

      # Sets the \api_version for URI generation
      ...
      def account_id
        ...
        if defined?(@account_id)
          @account_id
        elsif superclass != Object && superclass.account_id
          superclass.account_id.dup.freeze
        end
      end

      # Sets the \account_id for URI generation
      ...
      def connection(refresh = false)
        if defined?(@connection) || superclass == Object
          ...
          @connection.account_id = account_id if account_id
      ...
      def element_name
        @element_name ||= model_name.element.dasherize
      end

      attr_writer :collection_name

      def collection_name
        @collection_name ||= model_name.element.dasherize
      end
      
      attr_writer :service_name

      def service_name
        @service_name ||= (superclass == Object ? model_name.element : superclass.service_name)
      end
      ...
      def parent_path(parent_id)
        "#{parent_model_name.dasherize}/#{parent_id}/" if parent_model_name
      end
...
#         puts prefix_options.inspect
#         puts query_options.inspect
        check_prefix_options(prefix_options)

        prefix_options, query_options = split_options(prefix_options) if query_options.nil?
        "#{api_prefix}#{prefix(prefix_options)}#{service_name}-service/#{collection_name.dasherize}/#{URI.parser.escape id.to_s}#{format_extension}#{query_string(query_options)}"
      end
      ...
      def new_element_path(prefix_options = {})
        "#{api_prefix}#{prefix(prefix_options)}#{service_name}-service/#{collection_name.dasherize}/new#{format_extension}"
      end
      ...
      def collection_path(prefix_options = {}, query_options = nil)
        parent_id=prefix_options.delete(:parent_id) if prefix_options
        check_prefix_options(prefix_options)
        prefix_options, query_options = split_options(prefix_options) if query_options.nil?
        "#{api_prefix}#{prefix(prefix_options)}#{service_name}-service/#{parent_path(parent_id)}#{collection_name.dasherize}#{format_extension}#{query_string(query_options)}"
      end


      def search_path(prefix_options = {}, query_options = nil)
        check_prefix_options(prefix_options)
        prefix_options, query_options = split_options(prefix_options) if query_options.nil?
        "#{api_prefix}#{prefix(prefix_options)}#{service_name}-service/#{collection_name.dasherize}-search#{format_extension}#{query_string(query_options)}"
      end
      ...
      def find(*arguments)
        ...
        case scope
          ...
          when :range then find_range(options,params)
      ...
      def all_ranged(*args)
        ...
        ranges.each do |range|
          results+=find(:range, range, options)
        end
        results
      end
      ...
      def ranges
        ...
        (format.decode(connection.options(path, headers).body)['getUris'] || []).collect do |uri|
          range = uri.gsub("/#{collection_name}/",'')
        end
      end
      ...
      def search(*args)
        ...
        path = search_path(prefix_options, query_options)
        instantiate_collection( (format.decode(connection.get(path, headers).body).getUris || []), prefix_options )      
      end
      ...
      private
        ...
        def find_every(options)
          ...
          rescue Brightpearl::ResourceNotFound
        ...
        def find_range(*args)
          puts "in find range", args.inspect
          begin
            range = args.slice!(0)
            options = args.slice!(0) || {}
            puts "options in range", options.inspect 
            prefix_options, query_options = split_options(options[:params]) 
            path = "#{collection_path}/#{range}#{query_string(query_options)}"
            instantiate_collection( (format.decode(connection.get(path, headers).body) || []))
          rescue Brightpearl::ResourceNotFound
            [] #TODO: Brightpearl shouldn't be returning ranges for items that can't be queried, see product-price OPTIONS 
          end
        end
        ...
        def find_single(scope, options)
          ...
          instantiate_record(format.decode(connection.get(path, headers).body).first, prefix_options)
        end

        def instantiate_collection(collection, prefix_options = {})
          #collection = collection.kind_of?(Hash) ? [collection] : collection 
    ...
    protected
      ...
      def collection_path(options = nil)
        options={:parent_id=>parent_id}.merge(options || {})
    ...
    private
      ...
      def find_or_create_resource_for(name)
        ...
        #TODO: remove hack because brightpearl sends back stuff like this:
        #"warehouses":{"2":{"defaultLocationId":0,"reorderLevel":1,"reorderQuantity":0}},"nominalCodeStock":"1001"}
        #where "2" can't be created as a resource
        name = "id_#{name}" if name.match(/^\d+$/)
      ...
      def create_resource_for(resource_name)
        resource = self.class.const_set(resource_name, Class.new(Brightpearl::Base))
